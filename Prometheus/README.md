...
- **Components**:
    - **Retrieval**: This module handles the scraping of metrics from different endpoints, which are discovered either through static configurations or dynamic service discovery methods.
...
- In world of Micro services we can scrape specific services from list of services to monitor using service discovery.
...
- Types of Exporters:
    - Common exporters include the **Node Exporter** (for hardware metrics), the **MySQL Exporter** (for database metrics), **kube-state-metrics Exporter** (for kubernetes) and various other application-specific exporters.
